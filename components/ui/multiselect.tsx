...
interface MultiSelectProps {
  ...
  customSelectedOptionRender?: (option: Option) => React.ReactNode;
  hasError?: boolean;
...
export function MultiSelect({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "min-h-10 rounded-md border text-base ring-offset-background focus-within:ring-[3px] md:text-sm dark:bg-input/30",
          hasError
      ...
      >
        <div className="relative flex flex-wrap gap-1 pr-5">
          ...
          <button
            ...
            title="Clear all"
            className={cn(
              "absolute right-0 inline-grid size-6 place-items-center rounded-full p-0 hover:bg-destructive/5 hover:text-destructive",
              (hideClearAllButton ||
                disabled ||
                selected.length < 1 ||
      ...
      <div className="relative">
        {open && (
          <CommandList
            className="absolute! top-2 z-50 w-full animate-in rounded-md border bg-popover text-popover-foreground shadow-md outline-none slide-in-from-top-1"
            onMouseLeave={() => {
          ...
          >
            {isLoading ? (
              ...
              <>
                ...
                <div className="px-3 py-1 text-sm">{optionsFooter}</div>
              </>
            )}
          </CommandList>
        )}
